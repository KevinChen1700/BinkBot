...
class Controller:
    ...
    def eggTelligenceRoutine(self):
        ...
        if self.startMovement is True:
            ...
            # also black line detection above all
            if distance < 10 and self.objDetector.qrScanner[0][0] == 0 and self.objDetector.egg[0] == 0:
                print("rotate left")
                self.mvcontroller.moveMotors(0, 511)
                ...
                print("rotate left (180 degrees)")
                self.motorState = [0, 511]
                sleep(2)
            else:
                if self.objDetector.egg[0] == 0:
                    # start looking for an egg
                    print("rotate the robot around its axis until it finds something for 4 seconds/360 degrees")
                    self.motorState = [0, 511]
                    sleep(4)
                # if it has found an egg
                if self.objDetector.egg[0] > 0:
                    if self.objDetector.egg[0] < 200:
                        print("drive to the left")
                        self.mvcontroller.moveMotors(0, 511)
                    elif self.objDetector.egg[0] > 400:
                        print("drive to the right")
                        self.mvcontroller.moveMotors(1023, 511)
                    elif 200 > self.objDetector.egg[0] > 400:
                        print("drive forward")
                        self.mvcontroller.moveMotors(511, 1023)
                    if distance < 10 and self.eggGrabbed is False:
                        print("then make it use the gripper to pick it up")
                        # the x is closing/opening of the gripper and y for up down movement
                        # start by moving down and open then close and last up again
                        self.mvcontroller.moveGripper(1023, 0)
                        sleep(1)
                        self.mvcontroller.moveGripper(0, 0)
                        sleep(1)
                ...
                if self.eggGrabbed is True:
                    print("look for the container by spinning left")
                    self.motorState = [0, 511]
                    sleep(4)
                    # if the QR code has been found
                    if self.objDetector.qrScanner[1] == self.cityString:
                        print("city drop off found")
                        # the qr is to the left, gets first value of qrScanner.
                        # This is an array with x and y coordinates of the QR code, we take the x value which is [0]
                        ...
                        # drive forward for 1 second and afterwards start the rotating and looking for QR again
